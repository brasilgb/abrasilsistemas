...
function BrandMark() {
    return (
        <a href="#inicio" className="flex items-center gap-2" aria-label="AB Sistemas — início">
            <img
    ...
    return (
        <>
            <Head title="Sistemas de gestão para negócios que querem crescer">
                ...
                <meta property="og:title" content="AB Sistemas — Tecnologia que transforma negócios" />
                <meta
                ...
                <meta name="twitter:title" content="AB Sistemas — Tecnologia que transforma negócios" />
                <meta
            ...
            <div className="min-h-screen overflow-hidden bg-[#060d19] text-white selection:bg-sky-300 selection:text-slate-950">
                <header className="fixed inset-x-0 top-0 z-50 border-b border-white/8 bg-[#060d19]/80 backdrop-blur-xl">
                    <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8 lg:px-12">
                        ...
                        <nav className="hidden items-center gap-8 text-sm font-medium text-slate-400 md:flex">
                            ...
                            <a href="#sob-medida" className="transition hover:text-white">Sob medida</a>
                            <a href="#tecnologia" className="transition hover:text-white">Nossa visão</a>
                        </nav>
                        <a
                            href="#solucoes"
                            className="hidden items-center gap-2 rounded-full border border-white/12 bg-white/6 px-4 py-2 text-xs font-semibold transition hover:border-sky-300/40 hover:bg-white/10 sm:flex"
                        >
                            Conheça os produtos
                ...
                <main>
                    <section id="inicio" className="relative flex min-h-[92svh] items-center pt-28">
                        <div className="pointer-events-none absolute inset-0 overflow-hidden">
                            ...
                            <div className="absolute inset-0 bg-[linear-gradient(rgba(148,163,184,0.045)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.045)_1px,transparent_1px)] bg-[size:72px_72px] [mask-image:linear-gradient(to_bottom,black,transparent_85%)]" />
                        ...
                        <div className="relative mx-auto grid w-full max-w-7xl items-center gap-14 px-5 py-20 sm:px-8 lg:grid-cols-[1.12fr_0.88fr] lg:px-12">
                            <div>
                                ...
                                <p className="mt-8 max-w-2xl text-base leading-7 text-slate-400 sm:text-xl sm:leading-8">
                                    Na AB Sistemas, criamos plataformas que organizam rotinas, conectam equipes e dão clareza para empresas tomarem decisões melhores.
                                </p>
                                <div className="mt-10 flex flex-col gap-3 sm:flex-row">
                                    <a href="#solucoes" className="inline-flex h-13 items-center justify-center gap-2 rounded-xl bg-white px-6 text-sm font-bold text-slate-950 transition hover:bg-sky-100">
                                        Explorar soluções
                            ...
                            <div className="relative mx-auto w-full max-w-lg lg:max-w-none">
                                ...
                                <div className="relative grid gap-3 rounded-[2rem] border border-white/10 bg-white/[0.035] p-3 shadow-2xl shadow-black/30 backdrop-blur-sm">
                                    {products.map((product, index) => (
                                        <a key={product.id} href={`#${product.id}`} className="group flex items-center gap-4 rounded-2xl border border-white/8 bg-[#0a1424]/80 p-4 transition hover:border-sky-300/25 hover:bg-white/[0.06] sm:p-5">
                                            <div className="grid size-16 shrink-0 place-items-center overflow-hidden rounded-2xl border border-white/80 bg-white p-1.5 sm:size-20">
                                                <img src={product.logo} alt={product.logoAlt} className="size-full object-contain" />
                                            </div>
                                            <div className="min-w-0 flex-1">
                                                <span className="text-[9px] font-bold tracking-[0.17em] text-slate-500 uppercase">Solução 0{index + 1}</span>
                                                <h2 className="mt-1 text-xl font-bold tracking-tight sm:text-2xl">{product.name}</h2>
                                                <p className="mt-1 truncate text-xs text-slate-400 sm:text-sm">{product.label}</p>
                                            </div>
                                            <ArrowUpRight className="size-5 shrink-0 text-slate-600 transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-sky-300" />
                                        </a>
                                    ))}
                                </div>
                            </div>
                    ...
                    <section id="solucoes" className="border-y border-white/8 bg-[#08111f] py-24 sm:py-32">
                        <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
                            <div className="max-w-3xl">
                                <p className="text-xs font-bold tracking-[0.22em] text-sky-300 uppercase">Nosso ecossistema</p>
                                <h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-balance sm:text-6xl">Soluções especializadas. Resultados concretos.</h2>
                                <p className="mt-6 text-lg leading-8 text-slate-400">Cada produto nasce de uma operação real e resolve desafios específicos com profundidade, simplicidade e foco em produtividade.</p>
                            </div>

                            <div className="mt-16 space-y-8">
                                {products.map((product, index) => {
                                    return (
                                        <article id={product.id} key={product.id} className="scroll-mt-28 overflow-hidden rounded-[2rem] border border-white/10 bg-[#0b1627] shadow-2xl shadow-black/20">
                                            <div className="grid lg:grid-cols-[0.92fr_1.08fr]">
                                                <div className={`relative overflow-hidden p-7 sm:p-10 lg:p-12 ${index === 1 ? 'lg:order-2' : ''}`}>
                                                    <div className={`absolute inset-0 ${product.tone === 'sky' ? 'bg-gradient-to-br from-sky-500/12 via-transparent to-blue-600/8' : 'bg-gradient-to-br from-violet-500/12 via-transparent to-fuchsia-600/8'}`} />
                                                    <div className="relative">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                <p className={`text-[10px] font-bold tracking-[0.2em] uppercase ${product.tone === 'sky' ? 'text-sky-300' : 'text-violet-300'}`}>{product.label}</p>
                                                                <h3 className="mt-1 text-3xl font-bold tracking-tight sm:text-4xl">{product.name}</h3>
                                                            </div>
                                                        </div>
                                                        <h4 className="mt-9 max-w-xl text-3xl leading-tight font-semibold tracking-[-0.03em] text-balance sm:text-4xl">{product.headline}</h4>
                                                        <p className="mt-5 max-w-xl text-base leading-7 text-slate-400">{product.description}</p>
                                                        <a href={product.url} target="_blank" rel="noreferrer" className={`mt-8 inline-flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-slate-950 transition ${product.tone === 'sky' ? 'bg-sky-300 hover:bg-sky-200' : 'bg-violet-300 hover:bg-violet-200'}`}>
                                                            Conhecer {product.name}
                                                            <ArrowUpRight className="size-4" />
                                                        </a>
                                                        <p className="mt-3 text-xs text-slate-600">{product.domain}</p>
                                                    </div>
                                                </div>

                                                <div className={`border-white/8 bg-[#07101d] p-7 sm:p-10 lg:p-12 ${index === 1 ? 'lg:order-1 lg:border-r' : 'lg:border-l'}`}>
                                                    <div className="grid grid-cols-3 gap-2 sm:gap-3">
                                                        {product.stats.map((stat) => {
                                                            const StatIcon = stat.icon;
                                                            return (
                                                                <div key={stat.text} className="rounded-2xl border border-white/8 bg-white/[0.035] p-3 sm:p-4">
                                                                    <StatIcon className={`size-5 ${product.tone === 'sky' ? 'text-sky-300' : 'text-violet-300'}`} />
                                                                    <p className="mt-4 text-[10px] leading-4 font-semibold text-slate-300 sm:text-xs sm:leading-5">{stat.text}</p>
                                                                </div>
                                                            );
                                                        })}
                                                    </div>
                                                    <div className="mt-8">
                                                        <p className="mb-5 text-[10px] font-bold tracking-[0.2em] text-slate-500 uppercase">Principais recursos</p>
                                                        <ul className="grid gap-4 sm:grid-cols-2">
                                                            {product.features.map((feature) => (
                                                                <li key={feature} className="flex items-start gap-3 text-sm leading-6 text-slate-300">
                                                                    <span className={`mt-1 grid size-5 shrink-0 place-items-center rounded-full ${product.tone === 'sky' ? 'bg-sky-400/12 text-sky-300' : 'bg-violet-400/12 text-violet-300'}`}>
                    ...
                    <section id="tecnologia" className="relative py-24 sm:py-32">
                        ...
                        <div className="relative mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
                            <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
                                <div>
                                    <p className="text-xs font-bold tracking-[0.22em] text-sky-300 uppercase">Como construímos</p>
                                    <h2 className="mt-4 text-4xl font-semibold tracking-[-0.04em] text-balance sm:text-5xl">Tecnologia a serviço de quem faz acontecer.</h2>
                                    <p className="mt-6 text-base leading-7 text-slate-400">Não criamos apenas telas. Entendemos rotinas, eliminamos atritos e transformamos processos em experiências claras.</p>
                                </div>
                                <div className="grid gap-4 sm:grid-cols-2">
                                    {principles.map((principle) => {
                                        const Icon = principle.icon;
                                        return (
                                            <div key={principle.title} className="group rounded-2xl border border-white/8 bg-white/[0.03] p-6 transition hover:-translate-y-1 hover:border-sky-300/20 hover:bg-white/[0.05]">
                    ...
                    <section id="sob-medida" className="scroll-mt-20 border-y border-white/8 bg-[#08111f] py-24 sm:py-32">
                        <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
                            <div className="grid gap-12 lg:grid-cols-[0.88fr_1.12fr] lg:gap-20">
                                <div className="lg:sticky lg:top-32 lg:self-start">
                                    ...
                                    <h2 className="mt-6 text-4xl leading-tight font-semibold tracking-[-0.045em] text-balance sm:text-6xl">
                                        Tem uma ideia? Nós ajudamos a transformá-la em{' '}
                                    ...
                                    <p className="mt-6 max-w-xl text-base leading-7 text-slate-400 sm:text-lg sm:leading-8">
                                        Além das nossas plataformas, desenvolvemos sistemas e aplicativos personalizados para empresas que precisam resolver desafios únicos, digitalizar processos ou lançar um novo negócio.
                                    </p>
                                    <div className="mt-8 rounded-2xl border border-sky-300/15 bg-sky-300/[0.045] p-5">
                                        <p className="text-sm leading-6 text-slate-300">
                                            <strong className="font-semibold text-white">Sua operação não precisa se adaptar a uma ferramenta genérica.</strong>{' '}
                                            Criamos a tecnologia para se adaptar à realidade e aos objetivos do seu negócio.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="relative">
                                    ...
                                    <div className="space-y-4">
                                        {customDevelopmentSteps.map((step) => {
                                            ...
                                            return (
                                                <article key={step.number} className="group relative rounded-3xl border border-white/8 bg-[#0b1627] p-6 pl-20 transition duration-300 hover:-translate-y-0.5 hover:border-violet-300/20 hover:bg-white/[0.045] sm:p-8 sm:pl-24">
                                                    ...
                                                    <span className="text-[10px] font-bold tracking-[0.2em] text-violet-300 uppercase">Etapa {step.number}</span>
                            ...
                            <div className="mt-16 flex flex-col items-start justify-between gap-6 rounded-3xl border border-white/10 bg-gradient-to-r from-sky-500/10 via-blue-500/8 to-violet-500/10 p-7 sm:p-10 md:flex-row md:items-center">
                                <div>
                                    <p className="text-xs font-bold tracking-[0.18em] text-sky-300 uppercase">Vamos conversar</p>
                                    <h3 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">Toda grande solução começa com uma boa conversa.</h3>
                                    <p className="mt-2 text-sm leading-6 text-slate-400">Conte o que você imagina. Nós ajudamos a encontrar o melhor caminho para construir.</p>
                                </div>
                                <div className="flex shrink-0 flex-col gap-3 sm:flex-row">
                                    <a href="mailto:contato@absistemas.com.br?subject=Tenho uma ideia para desenvolver" className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/6 px-5 py-3.5 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-white/10">
                                        <Mail className="size-4" />
                                        Enviar e-mail
                                    </a>
                                    <a href="https://wa.me/5551998931325?text=Ol%C3%A1%2C%20tenho%20uma%20ideia%20de%20sistema%20ou%20aplicativo%20para%20desenvolver." target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-5 py-3.5 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:bg-sky-100">
                    ...
                    <section className="px-5 py-24 sm:px-8 sm:py-32 lg:px-12">
                        <div className="relative mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-sky-300/15 bg-gradient-to-br from-blue-600/18 via-[#0b1729] to-violet-600/16 px-7 py-14 text-center sm:px-12 sm:py-20">
                            ...
                            <div className="relative">
                                <p className="text-xs font-bold tracking-[0.22em] text-sky-300 uppercase">AB Sistemas</p>
                                <h2 className="mx-auto mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.04em] text-balance sm:text-6xl">O próximo passo do seu negócio começa com clareza.</h2>
                                <p className="mx-auto mt-6 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">Escolha a solução que entende sua operação e descubra uma forma mais simples de trabalhar, acompanhar e crescer.</p>
                                <div className="mt-9 flex flex-col justify-center gap-3 sm:flex-row">
                                    {products.map((product) => (
                                        <a key={product.id} href={product.url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-6 py-3.5 text-sm font-bold text-slate-950 transition hover:-translate-y-0.5 hover:bg-sky-100">
                                            Conhecer {product.name}
                                            <ArrowUpRight className="size-4" />
                                        </a>
                                    ))}
                ...
                <footer className="border-t border-white/8 bg-[#040a13]">
                    <div className="mx-auto flex max-w-7xl flex-col gap-8 px-5 py-10 sm:px-8 md:flex-row md:items-end md:justify-between lg:px-12">
                        <div>
                            <BrandMark />
                            <p className="mt-5 max-w-sm text-xs leading-5 text-slate-500">Soluções digitais brasileiras para operações que querem trabalhar com mais controle, agilidade e visão.</p>
                        </div>
                        <div className="text-xs text-slate-500 md:text-right">
                            <div className="flex flex-col gap-2 md:items-end">
                                <a href="mailto:contato@absistemas.com.br" className="inline-flex items-center gap-2 transition hover:text-white">
                                    <Mail className="size-3.5" />
                                    contato@absistemas.com.br
                                </a>
                                <a href="https://wa.me/5551998931325" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 transition hover:text-white">
                                    <MessageCircle className="size-3.5" />
                                    (51) 99893-1325
                                </a>
                            </div>
                            <p className="mt-5 text-slate-600">VetorOS · VetorPet</p>
                            <p className="mt-2">© {new Date().getFullYear()} AB Sistemas. Todos os direitos reservados.</p>
                        </div>
                    </div>
                </footer>
            </div>
